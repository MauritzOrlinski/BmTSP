...
import time
from brute_force_unit import solve_brute_force


def standard_main(x, y, n_points, m_charge, n_uavs, n_generations, iterations_csip, n_population, left, mutation, crossover,
                  show_graph=False, save_graph=False, brute_force=False):
    points = []
    charging_stations = []
    ...
    compare_brute_force = -1
    time_for_brute_force = 0
    if brute_force:
        start_time = time.time()
        p = solve_brute_force(points, charging_stations, n_uavs)
        end_time = time.time()
        time_for_brute_force = end_time - start_time
        if p == []:
            print("No solution exists")
        else:
            compare_brute_force = needed_time(p)

    start_time = time.time()
    p = solve(points, charging_stations, n_uavs, n_population, left, mutation, crossover, n_generations, iterations_csip)
    end_time = time.time()
    ...
    if compare_brute_force:
        print("Compute Time Brute Force", time_for_brute_force)
        print("Best possible time: ", compare_brute_force)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_points', type=int, default=60, help='number of points')
    parser.add_argument('--m_charge', type=int, default=15, help='number of charging stations')
    parser.add_argument('--n_uavs', type=int, default=6, help='number of UAVs')
    ...
    parser.add_argument('--iter_csip', type=int, default=40, help='iterations for CSIP Solver')
    ...
    standard_main(args.x, args.y, args.n_points, args.m_charge, args.n_uavs, args.n_generations, args.iter_csip, args.n_population,
